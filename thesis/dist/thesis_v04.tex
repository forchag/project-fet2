...
arithmetic~\cite{ding1996remainder, parhami2010computer}. A second property
of Equation~\ref{eq:crt_reconstruction} matters as much as its speed:
because each residue $r_i$ individually is close to uniformly distributed
over $[0,m_i)$ and discloses little about $x$ on its own, a residue set
functions as a natural threshold structure, exact reconstruction from a
complete or near-complete set, and negligible individual disclosure from
any one residue in isolation. Goldreich, Ron and Sudan formalised the
error-tolerant case, showing that $x$ remains recoverable from a strict
subset of residues under bounded corruption, "Chinese remaindering with
...
theory~\cite{shoup2009computational}. That subset-recovery property is
exploited twice in this thesis for two different purposes, transmission
resilience in Chapter Three and the confidentiality argument taken up in
Section~\ref{subsec:lit_privacy} below, and the literature reviewed here
establishes both properties independently without connecting them.
...
\gls{bft} protocols such as \gls{pbft} require $O(n^2)$ messages per
decision to tolerate the same $f$ under arbitrary,
not merely crash, node behaviour~\cite{castro1999practical,
ongaro2014raft}. Linear-complexity \gls{bft} variants such as HotStuff
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[leaf, fill=purple!4] (l2) at (2.2,0.2)
      {\gls{pbft}\\$O(n^2)$ msgs/decision\\\cite{castro1999practical}};
...
established that an individual \gls{crt} residue discloses little about
the value it derives from, and that reconstruction tolerates a missing
residue under Goldreich, Ron and Sudan's error-tolerant
formulation~\cite{goldreich1999china}; that subset-recovery structure is
formally analogous to a threshold secret-sharing scheme, exact
reconstruction from a sufficient subset of shares and negligible
disclosure from any share short of that
threshold~\cite{shoup2009computational}, though the literature reviewed
here has developed the two constructions, residue decomposition and
threshold secret sharing, in separate communities without connecting
them for the purpose this thesis puts them to: treating a mechanism
introduced for transmission efficiency as a confidentiality substrate as
well. This is the argument Chapter One's rationale
(Section~\ref{sec:rationale}) makes and Chapter Four's hypothesis H5 tests.
...
\begin{enumerate}
  ...
  \item It reports \gls{cft} consensus performance measured on \gls{arm64}
        ...
        literature.
  \item It treats residue subset-disclosure, established formally
        by~\cite{goldreich1999china,shoup2009computational} but not
        previously exploited as a design resource, as a confidentiality
        substrate, connecting the transmission and privacy threads that
...
after descaling. Losing any one residue, say $r_3$, still leaves a unique
solution modulo $m_1 m_2 = 9{,}797$, comfortably above the scaled range of
every sensor type this thesis deploys, so the subset-recovery property of
Section~\ref{subsec:lit_crt} is not merely asymptotic; it holds at the
exact moduli this thesis uses.
