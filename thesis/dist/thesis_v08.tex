...
\begin{ubonehalfspace}\ubfrontheading{Dedication}
...
\end{singlespace}
\clearpage
 \begin{ubonehalfspace}\ubfrontheading{Acknowledgements}

\clearpage
 \end{ubonehalfspace}
\begin{ubonehalfspace}\ubfrontheading{Abstract}
...
\node[lane, anchor=east] at (-0.3,0) {Ledgers};
  ...
  \end{tikzpicture}
  \caption{Three lineages underlying this thesis, developed independently
...
\begin{figure}[htbp]
  ...
  \caption{Transmission-efficiency strategies for constrained \gls{iot}
           radios; this thesis occupies the open leaf on the right.}
...
multi-core processor, which, to the best of the candidate's knowledge, no
...
\begin{figure}[htbp]
  ...
  \end{tikzpicture}
  \caption{Eq.~\ref{eq:crt_reconstruction} applied to two parallel
           resources: processor cycles (prior work) versus radio
           sub-channels (this thesis).}
  \label{fig:lit-crt-pipelines}
\end{figure}

Figure~\ref{fig:crt-gateway-pipeline} shows the conceptual bottom half of
Figure~\ref{fig:lit-crt-pipelines} as an implemented gateway, reproduced from
the candidate's own architecture description in~\cite{forcha2026basedparallel}:
residues decoded, the signature verified, and the reconstructed reading
anchored on the permissioned ledger before an irrigation decision is taken.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/extracted/forcha2026-gateway-pipeline.png}
  \caption{Gateway-side residue decoding and ledger pipeline, reproduced
           from~\cite{forcha2026basedparallel}.}
  \label{fig:crt-gateway-pipeline}
\end{figure}
...
reproduces independently rather than merely assumes. Figure~\ref{fig:hrbac-trust-chain}
reproduces the enrolment chain as formalised in that prior work, root
certificate authority down through gateways to individual sensing nodes,
which is the structure Equations~\ref{eq:hrbac_eff} and~\ref{eq:hrbac_grant}
formalise and this thesis extends with per-node credentials.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/extracted/beloasone2025-hrbac-trust-chain.png}
  \caption{Hierarchical enrolment chain for an \gls{hrbac} deployment,
           reproduced from the candidate's own prior work
           \cite{beloasone2025hrbac}.}
...
load~\cite{forcha2026basedparallel}. Figure~\ref{fig:five-tier-architecture}
shows where that layer sits in the deployed system: the ledger occupies one
tier among five, with the radio link one tier below it.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{figures/extracted/forcha2026-five-tier-architecture.png}
  \caption{Five-tier system architecture, reproduced
           from~\cite{forcha2026basedparallel}.}
  \label{fig:five-tier-architecture}
\end{figure}
