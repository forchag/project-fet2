...
{\fontsize{12}{14}\selectfont (FE22P049)}\\
{\fontsize{12}{14}\selectfont M.Eng. in Telecommunications and Networks}
...
{\fontsize{12}{14}\selectfont Michael Ekonde SONE, PhD\\ Associate Professor}
...
\vspace{2\baselineskip}
\begin{center}
To my family, whose patience, faith and encouragement sustained this work.
\end{center}
\clearpage
...
Integrity and Privacy in Agricultural Data Processing''}, was carried out in
the Department of Computer Engineering, Faculty of Engineering and Technology,
University of Buea, in partial fulfilment of the requirements for the award of
the \textbf{Doctor of Philosophy Degree in Computer Engineering}.

\vspace{2.2\baselineskip}
\noindent\begin{tabularx}{\textwidth}{@{}X X@{}}
\textbf{Sign:} \hrulefill & \textbf{Date:} \hrulefill \\[0.5\baselineskip]
Michael Ekonde SONE, PhD & \\
Associate Professor & \\
(Supervisor) & \\[1.5\baselineskip]
\textbf{Sign:} \hrulefill & \textbf{Date:} \hrulefill \\[0.5\baselineskip]
Elie FUTE TAGNE, PhD & \\
Associate Professor & \\
(Co-Supervisor) &
...
\vfill
\begin{center}
\textbf{Candidate's declaration}\\[0.8\baselineskip]
I certify that this thesis is my original work and that all sources used have
been acknowledged.\\[1.5\baselineskip]
\textbf{Sign:} \rule{5cm}{0.4pt}\hspace{1cm}
\textbf{Date:} \rule{4cm}{0.4pt}\\[0.5\baselineskip]
FORCHA GLEN BELOA
\end{center}
\end{singlespace}
\clearpage
...
I express my sincere gratitude to my supervisor, Michael Ekonde SONE, PhD,
and my co-supervisor, Elie FUTE TAGNE, PhD, for their rigorous guidance,
constructive criticism and sustained encouragement throughout this research.
I also thank the staff of the Department of Computer Engineering and the
Faculty of Engineering and Technology, University of Buea, for providing the
academic environment in which the work was developed. I acknowledge the
colleagues and field collaborators who assisted with the sensor, gateway and
agricultural deployment activities. Finally, I am deeply grateful to my family
and friends for their patience, sacrifice and confidence in this work.

\clearpage
 \ubfrontheading{Abstract}

Agricultural Internet of Things deployments require trustworthy sensing,
timely actuation and auditable multi-stakeholder access, yet conventional
cloud-centred architectures expose field data to identity, integrity, privacy
and connectivity risks. This thesis develops a distributed blockchain-based
approach that combines Chinese Remainder Theorem residue transmission,
gateway-side parallel processing, Hyperledger Fabric and hierarchical
role-based access control. The design uses resource-constrained ESP32 sensing
nodes, LoRa links and Raspberry Pi gateways to protect the path from
measurement to ledger-backed decision. The literature review establishes that
prior studies normally evaluate sensing, compression, blockchain, access
control or irrigation separately, leaving little field evidence for their
combined operation. The resulting architecture therefore treats radio airtime,
reconstruction correctness, ledger latency, access-control overhead and
agricultural utility as one end-to-end system. Candidate-authored prior studies
report a reduction in per-packet airtime from 247 ms to 124 ms, 63 transactions
per second at 1.7 s mean latency on ARM64 gateways, and 99.4 percent
end-to-end reliability during a 61-day deployment. These results motivate the
thesis experiments and the explicit boundary conditions developed for partial
CRT reconstruction, credential bootstrapping and zone-scoped authorization.
The work contributes a reproducible architecture, a critical synthesis of the
relevant theory, and auditable appendices that connect mathematical,
networking and access-control claims to implementable procedures.
...
Figure~\ref{fig:three-properties} places the three shortfalls together. They are
not independent faults to be repaired one at a time; each is a consequence of
the same scarcity, and a remedy applied to one is paid for out of the budget
...
The problem this thesis addresses is therefore the following: how can identity,
...
There is also a structural reason to expect the combination to work. Residue
...
\subsection{Evidence from the Candidate's Prior Studies}
\label{subsec:lit_candidate_studies}

Two candidate-authored studies in the reference list provide direct empirical
and architectural evidence for the synthesis developed above. The first study
examines CRT as an arithmetic accelerator for blockchain validation and argues
that modular decomposition can expose independent work units to parallel
processors without changing the ledger's consensus rules
~\cite{beloa2025bitcoincrt}. Its importance to this thesis is conceptual:
it distinguishes parallelism inside a validator from parallelism in the radio
path, while retaining the same reconstruction identity. The present study
therefore does not treat CRT as generic compression. It asks which resource is
being relieved, processor cycles, packet airtime, gateway demodulation capacity
or memory, and evaluates the corresponding cost explicitly.

The second study implements the combined agricultural edge architecture shown
in Figure~\ref{fig:prior-five-tier}. Fifty ESP32 nodes communicate through
LoRa to four Raspberry Pi 4B gateways, which combine gateway-side processing,
BATMAN-adv mesh communication and Hyperledger Fabric
~\cite{beloasone2025hrbac}. The reported 61-day deployment reduced per-packet
airtime from 247 ms to 124 ms, sustained 63 transactions per second at 1.7 s
mean latency, and achieved 99.4 percent end-to-end reliability. These results
are evidence from a defined configuration, not universal bounds: the LoRa
spreading factor, payload layout, concentrator channel count, reporting window
and Fabric deployment all condition the measurements.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.94\textwidth]{figures/v06/prior-five-tier-architecture.png}
  \caption{Five-tier agricultural edge architecture and vertical control path
  reproduced from the candidate's prior field study
  ~\cite{beloasone2025hrbac}. The crop retains the original system and control
  views while removing unrelated page text.}
  \label{fig:prior-five-tier}
\end{figure}

Figure~\ref{fig:prior-crt-flow} makes the end-to-end trust boundary visible.
Residue encoding shortens the sensor payload, but it does not authenticate the
reading. Ed25519 signing, gateway verification, exact reconstruction for
complete residue sets, smart-contract validation, on-chain hashes and
off-chain time-series storage perform distinct functions. Treating them as one
blockchain-security mechanism would obscure where an adversary can alter,
replay, omit or infer data. The literature gap is therefore not the absence of
any one component; it is the absence of field studies that measure their
composition and state the assumptions between adjacent components.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/v06/prior-crt-blockchain-flow.png}
  \caption{Candidate-authored CRT-to-blockchain processing flow, cropped from
  the source paper~\cite{beloasone2025hrbac}. It separates transmission,
  signature verification, reconstruction, ledger validation and actuation.}
  \label{fig:prior-crt-flow}
\end{figure}

The trust-chain study further shows that identity assurance begins before
chaincode authorization. As Figure~\ref{fig:prior-hrbac-chain} illustrates,
the certificate authority enrolls the administrator, the administrator enrolls
gateways, and gateways provision sensor identities. This hierarchy narrows the
credential-issuing surface, but it also creates operational dependencies:
revocation propagation, gateway compromise and zone reassignment can invalidate
an otherwise correct role predicate. The appendices therefore include an
authorization matrix and a verification checklist linking each role, zone,
credential and ledger action to observable evidence.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/v06/prior-hrbac-trust-chain.png}
  \caption{Hierarchical RBAC certificate bootstrapping reproduced from the
  candidate's prior study~\cite{beloasone2025hrbac}.}
  \label{fig:prior-hrbac-chain}
\end{figure}
...
\chapter{Evidence and Reproducibility Checklist}
\label{app:evidence_checklist}

This appendix makes the evidence chain for Chapter Two auditable. It separates
claims taken from the literature, claims reported in the candidate's prior
papers, and claims that must be re-established by the thesis experiments.
Values are not promoted from a source paper to a universal system property
without recording the configuration that produced them.

\begin{table}[htbp]
\centering
\caption{Traceability from literature claim to thesis verification evidence.}
\label{tab:app-evidence}
\small
\begin{tabularx}{\textwidth}{@{}p{0.20\textwidth}p{0.25\textwidth}X@{}}
\toprule
\textbf{Claim} & \textbf{Boundary condition} & \textbf{Required evidence} \\
\midrule
CRT reconstructs exactly & Complete residue set; pairwise-coprime moduli;
encoded value below the modulus product & Test vectors, modulus set, encoded
domain, residue values and exact decoded outputs \\
Partial recovery is possible & Encoded value below the product of the received
moduli, or an explicitly reported alias rate & Packet-loss trace, received
subsets, success criterion and measured recovery rate \\
Per-packet airtime falls & Fixed spreading factor, bandwidth, coding rate,
preamble and payload schema & Raw and residue frame lengths, calculated
time-on-air and measured transmission timestamps \\
Ledger preserves integrity & Valid identities, endorsement policy and
hash-linked committed state & Enrollment records, endorsement results,
transaction IDs and independent hash comparison \\
HRBAC prevents cross-zone access & Correct role hierarchy, zone binding,
credential validity and token expiry & Allow/deny test matrix, certificate
subjects, timestamps, zone assignments and audit events \\
Field reliability exceeds 99 percent & Defined reporting window, retry policy
and end-to-end success criterion & Expected versus received readings, recovered
readings, duplicate handling and confidence interval \\
\bottomrule
\end{tabularx}
\end{table}

\section*{Figure provenance}
\addcontentsline{toc}{section}{Figure provenance}

The three raster figures introduced in Version 6 are crops from
\emph{CRT-Based Multi-Channel Residue Transmission with Gateway-Side Parallel
Demodulation for Distributed Edge Blockchain Computing: A Scalable IoT Cluster
Architecture for Arid Agriculture}, cited as
\cite{beloasone2025hrbac}. They are candidate-authored figures reproduced for
critical discussion. The crops remove surrounding page text only; labels,
arrows, values and topology are unchanged.

\section*{Reproduction sequence}
\addcontentsline{toc}{section}{Reproduction sequence}

\begin{enumerate}
  \item Record firmware revision, node count, reporting interval and sensor
        scaling before generating any residues.
  \item Verify that every modulus pair is coprime and that the encoded domain
        is smaller than the full modulus product.
  \item Capture raw and residue payloads at the same LoRa settings; compute and
        measure airtime for both.
  \item Reconstruct complete and incomplete residue sets separately. Report
        aliases as failures rather than silently selecting the least
        non-negative solution.
  \item Bind gateway and sensor identities to roles and zones, then execute the
        complete allow/deny matrix including expired and revoked credentials.
  \item Submit reconstructed records to Fabric, retain transaction identifiers
        and compare the committed hashes with the off-chain time-series records.
  \item Report denominators, observation windows and uncertainty for every
        reliability, latency, throughput and agricultural-efficiency result.
\end{enumerate}
