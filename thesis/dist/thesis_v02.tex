...
edge hardware.} \textit{(Bloom level: Create)}
...
\begin{enumerate}
  \item \textbf{Analyse} the identity, integrity and privacy requirements of
        ...
        stated capabilities and assumptions. \textit{(Analyse)}
  ...
  \item \textbf{Implement} the framework on a heterogeneous edge cluster of
        ...
        operating conditions. \textit{(Apply)}
  ...
  \item \textbf{Evaluate} the transmission, ledger and access-control layers
        ...
        validated against measured load. \textit{(Evaluate)}
\end{enumerate}
...
Figure~\ref{fig:roadmap} shows how the five chapters relate to the four
specific objectives of Section~\ref{subsec:specific_objectives}.

\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[
      ch/.style={draw, thick, rounded corners=2pt, align=center, font=\small,
                 text width=21mm, minimum height=13mm, inner sep=3pt},
      ob/.style={draw, dashed, rounded corners=2pt, align=center,
                 font=\footnotesize, text width=21mm, fill=black!3,
                 minimum height=9mm, inner sep=3pt},
      flow/.style={-{Stealth[length=2mm]}, thick},
      map/.style={-{Stealth[length=1.6mm]}, gray, thin}
    ]
    \node[ch] (c1) at (0,0)    {\textbf{One}\\Introduction};
    \node[ch] (c2) at (2.7,0)  {\textbf{Two}\\Literature};
    \node[ch] (c3) at (5.4,0)  {\textbf{Three}\\Methods};
    \node[ch] (c4) at (8.1,0)  {\textbf{Four}\\Results};
    \node[ch] (c5) at (10.8,0) {\textbf{Five}\\Discussion};

    \foreach \a/\b in {c1/c2, c2/c3, c3/c4, c4/c5} \draw[flow] (\a) -- (\b);

    \node[ob] (o1) at (2.7,-1.9)  {O1 Analyse};
    \node[ob] (o2) at (5.4,-1.9)  {O2 Design};
    \node[ob] (o3) at (8.1,-1.9)  {O3 Implement};
    \node[ob] (o4) at (10.8,-1.9) {O4 Evaluate};

    \draw[map] (c2.south) -- (o1.north);
    \draw[map] (c3.south) -- (o2.north);
    \draw[map] (c3.south east) -- (o3.north west);
    \draw[map] (c4.south) -- (o4.north);
  \end{tikzpicture}
  \caption{Organisation of the thesis and the chapter in which each specific
           objective is discharged.}
  \label{fig:roadmap}
\end{figure}
