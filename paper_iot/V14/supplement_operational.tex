...
Verifying \texttt{sig} against \texttt{digest} with \texttt{pubkey} must
succeed; verifying it against a digest of a modified \texttt{payload} must
fail. The executable checks are
\path{gateway/tests/test_signature_binding.py}; they cover the canonical
firmware vector and a single-byte modification.
