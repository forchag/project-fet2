...
// CheckAccess call this function makes -- nonce/certificate/role/zone
// evaluation and its own audit-entry write -- taken inside the chaincode
// with time.Now()/time.Since(). It is carried only in the transaction
// response payload, never written to world state via PutState, so it
// does not enter the endorsement read-write set: independent endorsing
// peers necessarily observe slightly different wall-clock durations for
// the same logical work, and including that value in state would make
// their RWsets diverge and endorsement fail. Client code (the gateway)
// is expected to record the value from whichever peer response it uses,
// alongside its own client-observed round-trip latency for the whole
// WriteSensorData transaction.
